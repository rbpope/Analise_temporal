...
plot(modelo_sazonal_tend_linear_proj, xlab="Tempo", ylab="Passageiros", xaxt="n" , ylim=c(1300, 2300), xlim=c(1991, 2006.25), bty="l", flty=2, main="Forecast from Seasonal & Tendencia regression model")
...
modelo_sazonal_tend_linear_final <- tslm(passageiros_ts ~ season + trend + I(trend^2))

#sumario do modelo
summary(modelo_sazonal_tend_linear_final)

#projeta os próximos 12 meses do futuro
modelo_sazonal_tend_linear_final_proj <- forecast(modelo_sazonal_tend_linear_final, h=36, level=0.95)

#plota o grafico da serie temporal de treinamento e teste
plot(modelo_sazonal_tend_linear_final_proj, xlab="Tempo", ylab="Passageiros", ylim=c(1300, 2800), xlim=c(1991, 2007), bty="l", flty=2,main="Forecast from Seasonal & Tendencia regression model")
